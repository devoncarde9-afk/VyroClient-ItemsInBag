// VyroClient Items in Bag - HOOKS initWithFrame
...
- (instancetype)initWithFrame:(CGRect)frame {
    self = %orig;
    if (self) {
        NSLog(@"[ItemsInBag] ACPanView initWithFrame called!");
        
        // Add section after a delay to let VyroClient finish setup
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)),
                       dispatch_get_main_queue(), ^{
            if (!objc_getAssociatedObject(self, kInjected)) {
                objc_setAssociatedObject(self, kInjected, @YES, OBJC_ASSOCIATION_RETAIN);
                NSLog(@"[ItemsInBag] Adding Items in Bag section...");
...
- (void)addItemsInBagSection {
    ...
    NSLog(@"[ItemsInBag] contentView=%@, scrollView=%@", cv, sv);
    
    if (!cv) {
        NSLog(@"[ItemsInBag] ERROR: contentView is nil! Searching for subviews...");
        for (UIView *subview in self.subviews) {
            if ([subview isKindOfClass:[UIScrollView class]]) {
                sv = (UIScrollView*)subview;
                NSLog(@"[ItemsInBag] Found scrollView: %@", sv);
            }
            if ([subview isKindOfClass:[UIView class]] && subview != sv) {
                cv = subview;
                NSLog(@"[ItemsInBag] Found contentView: %@", cv);
            }
        }
    }
    ...
    if (!cv && sv) {
        cv = sv;
        NSLog(@"[ItemsInBag] Using scrollView as contentView");
    }
    
    if (!cv) {
        NSLog(@"[ItemsInBag] ERROR: Still no contentView found!");
        return;
    }
    ...
    NSLog(@"[ItemsInBag] Adding section at y=%.0f, width=%.0f", y, W);
    
    CGFloat pad = 15, bH = 50, gap = 10;
    
    UIView *div = [[UIView alloc] initWithFrame:CGRectMake(pad, y, W-pad*2, 2)];
    div.backgroundColor = [UIColor colorWithWhite:1 alpha:0.3];
    [cv addSubview:div];
    y += 12;
    ...
    [cv addSubview:randomBtn];
    y += bH + 20;
    
    if (sv) {
        CGSize currentSize = sv.contentSize;
        sv.contentSize = CGSizeMake(currentSize.width, MAX(y, currentSize.height));
        NSLog(@"[ItemsInBag] Updated scroll size to: %@", NSStringFromCGSize(sv.contentSize));
    }
    
    NSLog(@"[ItemsInBag] ✅ Section added successfully!");
}
...
%ctor {
    NSLog(@"[ItemsInBag] ✅ Items in Bag Spawner loaded - hooking ACPanView initWithFrame");
}
